...
function TemplateTable({data} : { data: ITemplate[] }) {
    ...
    const onDelete = async (id: string) => {
        try {
            ...
            if (response.success) {
                message.success("Malli Pohjan poisto Onnistui")
            } else {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (value:boolean) => value ? "Kyllä" : "Ei"
        },
        {
            title: 'Created At',
            dataIndex: 'createdAt',
  ...
  return (
    <div>
        <Table columns={columns} dataSource={data.map((item, index) => ({ ...item, key: index }))} loading={loading} />
    </div>
  )
}
